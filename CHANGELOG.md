...
- **This changes output from four published pathways, not only the reader.** The fix is in the shared ODT parser, so `odf-kit/odt/read`, `odf-kit/odt/to-html`, `odf-kit/odt/to-markdown` and `odf-kit/odt/to-typst` all emit text they previously omitted — as do the legacy aliases that re-export them. Documents containing framed text boxes will produce more content than they did in 0.14.2. That is the point of the fix, but it is a behavioural output change in four places, and anyone diffing converted output across releases should know why it moved.
- **The limits, stated rather than left to be discovered.** The frame's own structure is not modelled — that a frame was there at all, its name, and its dimensions are still absent from the model. Paragraphs inside a text box concatenate with no separator, because the reader's inline model has nowhere to mark a block boundary, so a two-paragraph text box reads as `First paraSecond para` — and because the boundary is lost in the model, it is lost downstream too: `odtToHtml`, `odtToMarkdown` and `odtToTypst` each emit that as one unbroken run, not two paragraphs. Text-box paragraphs also render in the surrounding paragraph's formatting: their own styles are not resolved. Present and imperfect is a deliberate choice over absent, and each limit has a test pinning it so a later improvement is a visible change rather than a silent one.
- **Six of the eight representations a `draw:frame` may hold are still dropped silently, and which one you lose depends on what else the frame contains.** `draw:object`, `draw:object-ole`, `draw:applet`, `draw:floating-frame`, `draw:plugin` and `table:table` yield nothing on their own — a table inside a frame still vanishes whole — while the same content sitting beside an image or a text box leaves you with the image or the text instead. This is recorded, not fixed: #94 covers the text case only.
