...
/** Wrap inline nodes in a single paragraph body for renderOdtHtml. */
...
describe("renderOdtHtml Tier 2 — hidden spans", () => {
  test("hidden span produces no output", () => {
    const html = renderOdtHtml(para([{ text: "visible" }, { text: "hidden", hidden: true }]), {
      fragment: true,
    });
  ...
  test("hidden span does not emit a <span> wrapper", () => {
    const html = renderOdtHtml(para([{ text: "gone", hidden: true }]), { fragment: true });
...
describe("renderOdtHtml Tier 2 — SpanStyle inline CSS", () => {
  test("fontColor emits color CSS property", () => {
    const html = renderOdtHtml(para([{ text: "red", style: { fontColor: "#ff0000" } }]), {
      fragment: true,
    });
  ...
  test("fontSize emits font-size in pt", () => {
    const html = renderOdtHtml(para([{ text: "big", style: { fontSize: 18 } }]), {
      fragment: true,
    });
  ...
  test("fontFamily emits font-family CSS property", () => {
    const html = renderOdtHtml(para([{ text: "arial", style: { fontFamily: "Arial" } }]), {
      fragment: true,
    });
  ...
  test("textTransform emits text-transform CSS property", () => {
    const html = renderOdtHtml(para([{ text: "caps", style: { textTransform: "uppercase" } }]), {
      fragment: true,
    });
  ...
  test("fontVariant emits font-variant CSS property", () => {
    const html = renderOdtHtml(para([{ text: "sc", style: { fontVariant: "small-caps" } }]), {
      fragment: true,
    });
  ...
  test("letterSpacing emits letter-spacing CSS property", () => {
    const html = renderOdtHtml(para([{ text: "spaced", style: { letterSpacing: "0.05em" } }]), {
      fragment: true,
    });
  ...
  test("span with no style properties emits no <span> wrapper", () => {
    const html = renderOdtHtml(para([{ text: "plain" }]), { fragment: true });
...
describe("renderOdtHtml Tier 2 — cell and row styles", () => {
...
describe("renderOdtHtml Tier 2 — images", () => {
  ...
  test("renders an image as <img> with data URI src", () => {
    const html = renderOdtHtml(para([baseImage]), { fragment: true });
  ...
  test("title renders as alt attribute", () => {
    ...
    const html = renderOdtHtml(para([image]), { fragment: true });
...
describe("renderOdtHtml Tier 2 — notes", () => {
  ...
  test("renders a superscript citation anchor", () => {
    const html = renderOdtHtml(para([footnote]), { fragment: true });
...
describe("renderOdtHtml Tier 2 — fields", () => {
  ...
  test("fixed field renders identically to live field (value-only output)", () => {
    ...
    const liveHtml = renderOdtHtml(para([live]), { fragment: true });
    const fixedHtml = renderOdtHtml(para([fixed]), { fragment: true });
...
describe("renderOdtHtml Tier 2 — mixed inline content", () => {
  test("paragraph with text, bookmark, and field renders all three", () => {
    ...
    const html = renderOdtHtml(para(spans), { fragment: true });
